...
class PriceSelectionViewModel(val itemView: View) {
    fun bind(model: PriceSelectionModel, onPriceSelected: (AssetPrice, BigDecimal) -> Unit) {

        itemView.amountText.text = model.amount.toLabelAndCurrencyDisplayString(model.label)

        val adapter = ArrayAdapter(itemView.context, android.R.layout.simple_spinner_item, model.prices.map { it.toDisplayString() })
